...
- **The Token Fund is the one gift lane.** `/fuel` sells AI tokens for work (tiers in `lib/fuel.ts`; the $5 floor exists because under it card fees start winning). It rides the store's Stripe Checkout (`app/api/checkout/fuel`), writes a `support_intents` row (purpose `site`, message prefixed `Token Fund`) at checkout and flips it to `paid` from the webhook and `/fuel/thanks`. One-time money lands in `donations` (campaign `fuel`) from `checkout.session.completed`; the monthly "Keeper" lane is a Stripe subscription whose every invoice lands from `invoice.paid` via `recordFuelInvoice` (invoice id in `stripe_session_id`), so never also record a subscription session. The ledger row with `fuel_role = 'overage'` in `funding_line_items` is the ask and the meter target (the "month" tier), never a typed number; rows with `fuel_role = 'subscription'` are what Ryan pays himself, shown on `/fuel` as context and excluded from every public goal (`funding_buckets()`, `lib/funding.ts`). "What a dollar buys" is arithmetic on Anthropic's published Fable 5.1 rates and the token mix measured in `docs/usage-receipts-2026-09.md` (`lib/fuel.ts`), and the page calls it an estimate until a month runs on credits. Never type a bill, a per-hour, or a per-article cost. `/api/fuel/status` feeds the live meter. No other donate routes. Every post page renders `components/FuelAsk.tsx` under the body (the hard-coded ask, in Ryan's words); never paste a copy into a post body. `FUEL_TIME_FLOOR_CENTS` ($50) is the floor for anything that costs Ryan's time; tiers under it buy fuel and a name on the wall only.
- **Live radar is client-only.** `components/LiveVisitorRadar.tsx` (d3-geo + world-atlas + us-atlas) must be loaded with `next/dynamic` and `ssr: false` inside `RadarFrame`, which server-renders the headline. Rendering the map on the server once put a 900KB path string into `/the-map-room`. Pings plot near the visitor's city from Vercel's `x-vercel-ip-latitude/longitude` headers, rounded to 0.1° (about 11 km) in `app/api/track-pageview` and again in `live_visitor_pings()`, and never outside the state Vercel named; without coordinates they sit inside the state (`lib/radar-geo.ts`). No public surface shows a visitor's path, page trail, or raw coordinates; `live_visitor_session_detail` is service-role only.
