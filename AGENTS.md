...
- **The Token Fund is the one gift lane.** `/fuel` sells AI tokens for work (tiers in `lib/fuel.ts`; floor exists because a tiny gift costs more in card fees than it gives). It rides the store's Stripe Checkout (`app/api/checkout/fuel`), writes a `support_intents` row (purpose `site`, message prefixed `Token Fund`) at checkout and flips it to `paid` from the webhook and `/fuel/thanks`. Money lands in `donations` with campaign `fuel`. The "month" tier is the AI bill in `funding_line_items`, never a typed number. No other donate routes.
