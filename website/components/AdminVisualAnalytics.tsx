...
function CompactTooltip({
  ...
  return (
    <div className="rounded-lg border border-[var(--color-line)] bg-[var(--color-surface-2)] px-3 py-2 text-xs text-[var(--color-cream)] shadow-2xl">
    ...
    <div className="mt-3 grid gap-1.5 text-xs">
      {data.map((slice) => (
        <div key={slice.name} className="flex items-center justify-between gap-3">
          ...
          <span className="font-mono font-black tabular-nums text-[var(--color-ink)]">{fmt(slice.value)}</span>
        </div>
      ))}
    </div>
  );
...
function toneClass(tone: VisualSystemLine["tone"]) {
  if (tone === "good") return "border-[var(--color-live)]/50 bg-[var(--color-live)]/10 text-[var(--color-live)]";
  if (tone === "watch") return "border-[var(--color-gold-light)]/50 bg-[var(--color-gold-light)]/10 text-[var(--color-gold-light)]";
  if (tone === "bad") return "border-[var(--color-tag-severe)]/50 bg-[var(--color-tag-severe)]/10 text-[var(--color-tag-severe)]";
  return "border-[var(--color-line-soft)] bg-[var(--color-surface-2)] text-[var(--color-ink-soft)]";
}
...
export function AdminVisualAnalytics({
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] text-[var(--color-ink)] shadow-2xl">
      <div className="border-b border-[var(--color-line-soft)] bg-[var(--color-surface-2)] px-4 py-4 sm:px-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-2xl">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--color-live)]">
              Visual intelligence wall
            </p>
            <h2 className="mt-1 text-xl font-black tracking-tight text-[var(--color-ink)] sm:text-2xl">
              Traffic, sources, action, money, and system health in one read.
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-[var(--color-ink-soft)]">{insight}</p>
          </div>
          <Link
      ...
      <div className="grid gap-px bg-[var(--color-line-soft)] lg:grid-cols-[1.25fr_0.75fr]">
        <div className="bg-[var(--color-surface)] p-4 sm:p-5">
          <div className="flex items-baseline justify-between gap-3">
            <div>
              <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">
            ...
            <span className="rounded-full bg-[var(--color-blue-soft)] px-3 py-1 text-[10px] font-black uppercase text-[var(--color-blue-ink)]">
              14d
            </span>
          </div>
          <div className="mt-4 h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={daily} margin={{ top: 8, right: 8, bottom: 0, left: -24 }}>
                ...
                <XAxis dataKey="label" stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} minTickGap={22} />
                <YAxis stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} allowDecimals={false} />
                <Tooltip content={<CompactTooltip />} cursor={{ stroke: "var(--color-line-soft)", strokeDasharray: "4 4" }} />
        ...
        <div className="grid gap-px bg-[var(--color-line-soft)] sm:grid-cols-2 lg:grid-cols-1">
          <div className="bg-[var(--color-surface)] p-4 sm:p-5">
            <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">Source mix</h3>
          ...
          <div className="bg-[var(--color-surface)] p-4 sm:p-5">
            <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">Device split</h3>
            <div className="mt-3 h-44">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={deviceMix} margin={{ top: 8, right: 4, bottom: 0, left: -24 }}>
                  <CartesianGrid stroke="var(--color-line-soft)" vertical={false} />
                  <XAxis dataKey="name" stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} />
                  <YAxis stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} allowDecimals={false} />
                  <Tooltip content={<CompactTooltip />} />
      ...
      <div className="grid gap-px bg-[var(--color-line-soft)] lg:grid-cols-2">
        <div className="bg-[var(--color-surface)] p-4 sm:p-5">
          <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">Attention to money path</h3>
          <p className="mt-1 text-xs text-[var(--color-muted)]">
            ...
            {funnel.map((step, index) => {
              ...
              return (
                <div key={step.name}>
                  ...
                  <div className="mt-1 h-3 overflow-hidden rounded-full bg-[var(--color-line-soft)]">
                    <div className="h-full rounded-full" style={{ width: `${width}%`, background: step.fill }} />
                  </div>
                  <p className="mt-1 text-[11px] text-[var(--color-muted)]">{step.hint}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-[var(--color-surface)] p-4 sm:p-5">
          <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">Top pages as a picture</h3>
          <p className="mt-1 text-xs text-[var(--color-muted)]">
          ...
          <div className="mt-4 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={content} layout="vertical" margin={{ top: 0, right: 10, bottom: 0, left: 18 }}>
                ...
                <XAxis type="number" stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} allowDecimals={false} />
                <YAxis type="category" dataKey="name" width={88} stroke="var(--color-muted)" tickLine={false} axisLine={false} fontSize={11} />
                <Tooltip content={<CompactTooltip />} />
      ...
      <div className="grid gap-px bg-[var(--color-line-soft)] md:grid-cols-3">
        {systems.map((system) => (
          <div key={system.name} className="bg-[var(--color-surface)] p-4 sm:p-5">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-black tracking-tight text-[var(--color-ink)]">{system.name}</h3>
            ...
            <p className="mt-2 font-mono text-2xl font-black tabular-nums text-[var(--color-ink)]">{system.value}</p>
            <p className="mt-1 text-xs leading-snug text-[var(--color-muted)]">{system.detail}</p>
          </div>
        ))}
      </div>
    </section>
