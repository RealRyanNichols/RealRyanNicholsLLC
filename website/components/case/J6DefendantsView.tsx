...
import { cleanCaseSearch, type CasePerson } from "@/lib/case";
...
export function J6DefendantsView({
  ...
  // The directory filtered on the cleaned query (getJ6PeoplePage), so the
  // marks use the same needle: "John  Doe" finds and marks "John Doe".
  const needle = cleanCaseSearch(q);
                ...
                <div className="flex flex-1 flex-col p-4">
                  ...
                  {p.role ? (
                    <p className="mt-1 text-sm leading-snug text-[var(--color-muted)]">
                      <Highlight text={p.role} q={needle} />
                    </p>
                  ) : null}
                  <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] text-[var(--color-muted)]">
                    <span className="rounded-md border border-[var(--color-line-soft)] bg-[var(--color-paper)] px-2 py-1">
                      {p.case_number ? <Highlight text={p.case_number} q={needle} /> : "Case # needed"}
                    </span>
