...
export async function RelatedPosts({ slugs }: { slugs: string[] }) {
  ...
  return (
    <aside className="not-prose my-8 rounded-lg border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
        ...
        {ordered.map((r) => {
          ...
          return (
            <Link
              key={r.slug}
              href={`/posts/${r.slug}`}
              className="group overflow-hidden rounded-md border border-[var(--color-line)] bg-[var(--color-surface-2)] no-underline transition hover:border-[var(--color-gold)]"
            >
              {img ? (
