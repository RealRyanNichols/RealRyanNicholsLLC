...
export function CommentForm({ postId, signedIn }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] p-5">
      <label htmlFor="comment" className="mb-2 block text-sm font-bold text-[var(--color-ink-soft)]">
        Add your comment
      </label>
      <textarea
        id="comment"
        ...
        className="w-full rounded-lg border border-[var(--color-line)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-ink)] focus:outline-none focus:border-[var(--color-accent)]"
      />
