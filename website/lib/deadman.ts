...
export function buildInitialCustodyBulletin(input: {
  ...
  const lines = [
    ...
    "## Verified facts",
    "",
    `At **${time} Central Time**, an authorized operator activated this protocol after double confirmation using **${label}**.`,
    ...
    "",
    "The activating operator's full report is preserved in the private incident log. It is not copied into this article because a free-form description is not, by itself, proof of every detail it may contain.",
    "",
    ...
    "",
    "The exact legal basis, operative order, alleged violation, booking information, counsel's response, and next hearing must be confirmed from original sources. The reporting will compare the government's stated basis against controlling orders, release conditions, docket entries, recordings, exculpatory evidence, favorable context, and contrary evidence. A single repost or social-media tag is not proof; same-day original reporting from established newsrooms may be used when identity and current custody are corroborated.",
    "",
